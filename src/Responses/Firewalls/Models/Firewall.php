...
final readonly class Firewall implements ResponseContract
{
    ...
    {
        return new self(
            array_map(fn (array $appliedTo): \HetznerCloud\Responses\Firewalls\Models\AppliedTo => AppliedTo::from($appliedTo), $attributes['applied_to']),
            $attributes['created'],
            $attributes['id'],
            $attributes['labels'],
            $attributes['name'],
            array_map(fn (array $rule): \HetznerCloud\Responses\Firewalls\Models\FirewallRule => FirewallRule::from($rule), $attributes['rules']),
        );
    }
