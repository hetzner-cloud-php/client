...
describe(ClientFake::class, function () {
    ...
    it('returns a fake response', function () {
        // Arrange
        $data = CreateServerFixture::data()['server'];
        $server = $data['server'];
        $fake = new ClientFake([CreateServerResponse::from($data)]);
        ...
        // Assert
        expect($response->server->name)->toBe($server['name']);
        expect($response->server->image->name)->toBe($server['image']['name']);
        expect($response->server->serverType->name)->toBe($server['server_type']['name']);
    });
